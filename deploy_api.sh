...
cloudformation_package() {
    aws cloudformation package \
        ...
        --output-template-file ${1}-packaged-template.yaml
}

cloudformation_deploy() {
    aws cloudformation deploy \
      --template-file ${1}-packaged-template.yaml \
      --s3-bucket $DEPLOY_BUCKET_NAME \
...
# call
# add functions
cloudformation_package "function"
cloudformation_deploy "function"
if [ $? -ne 0 ]; then
...
# api-template
cloudformation_package "api"
cloudformation_deploy "api"
if [ $? -ne 0 ]; then
    echo "Failed api"
    exit 1
fi
...
# add permissions
cloudformation_package "permission"
cloudformation_deploy "permission"
if [ $? -ne 0 ]; then
